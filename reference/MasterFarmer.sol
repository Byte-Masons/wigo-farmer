...
contract WigoToken is ERC20("WigoSwap Token", "WIGO") {
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(totalSupply().add(_amount) <= MAX_SUPPLY, "ERC20: minting more than maxSupply");
        _mint(_to, _amount);
...
contract MasterFarmer is Ownable {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < REWARD_MULTIPLIER.length - 1; i++) {
            uint256 changeMultiplierAtTime = _changeMultiplierAfterTime.mul(i + 1).add(_startBlockTime);
            CHANGE_MULTIPLIER_AT_TIME.push(changeMultiplierAtTime);
        }
        FINISH_BONUS_AT_TIME = _changeMultiplierAfterTime.mul(REWARD_MULTIPLIER.length - 1).add(_startBlockTime);

        // staking pool
        poolInfo.push(
            PoolInfo({lpToken: _wigo, allocPoint: 1000, lastRewardBlockTime: startBlockTime, accWigoPerShare: 0})
        );
    ...
    function checkForDuplicate(IERC20 _lpToken) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].lpToken != _lpToken, "Add: pool already exists!!!!");
        }
    }
    ...
    function wigoCanMint(uint256 _amount) internal view returns (uint256 wigoReward) {
        uint256 canMint = wigo.maxSupply().sub(wigo.totalSupply()).mul(1000).div(
            1000 + PERCENT_FOR_TREASURY + PERCENT_FOR_DEV
        );
        if (canMint < _amount) {
            wigoReward = canMint;
        } else {
    ...
    function pendingWigo(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardBlockTime && lpSupply != 0) {
            ...
            uint256 rewardAmount = multiplier.mul(wigoPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            uint256 wigoReward = wigoCanMint(rewardAmount);
            accWigoPerShare = accWigoPerShare.add(wigoReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (wigo.totalMinted() < wigo.maxSupply()) {
            ...
            wigo.mint(treasuryAddr, wigoReward.mul(PERCENT_FOR_TREASURY).div(1000));
        }
        wigo.mint(address(bank), wigoReward);
        pool.accWigoPerShare = pool.accWigoPerShare.add(wigoReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlockTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accWigoPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        require(userBankBalance >= _amount, "withdraw: You do not have enough xWIGO!");
        require(user.amount >= _amount, "withdraw: not good");
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accWigoPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        if (_pid == 0) {
            uint256 userBankBalance = bank.balanceOf(msg.sender);
            require(userBankBalance >= user.amount, "EmergencyWithdraw: You do not have enough xWIGO!");
